...
const HandleBrowserChange = (url) => {
    ...
    const marketPattern = /^https:\/\/steamcommunity\.com\/market\/.*$/;
    const communityPattern = /^https:\/\/steamcommunity\.com\/(?!market).*$/;

    if (activityPattern.test(url))       UpdateSelectedNode("activity");
    else if (storePattern.test(url))     UpdateSelectedNode("store");
    else if (marketPattern.test(url))    UpdateSelectedNode("market"); 
