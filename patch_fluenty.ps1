...
# 3. If action is Install, download theme from user's GitHub
if ($action_choice -eq "Install") {
    Write-Host (([char[]](1057,1082,1072,1095,1080,1074,1072,1085,1080,1077,32,1072,1088,1093,1080,1074,1072,32,1090,1077,1084,1099,32,70,108,117,101,110,116,121,32,1089,32,1074,1072,1096,1077,1075,1086,32,71,105,116,72,117,98,46,46,46) -join "")) -ForegroundColor Cyan
    $zip_url = "https://github.com/DarkAssassinUA/fluenty-luatools-patcher/archive/refs/heads/main.zip"
    $temp_zip = Join-Path $env:TEMP "fluenty-main.zip"
    try {
        Invoke-WebRequest -Uri $zip_url -OutFile $temp_zip -ErrorAction Stop
    } catch {
        Write-Host (([char[]](1054,1096,1080,1073,1082,1072,32,1087,1088,1080,32,1089,1082,1072,1095,1080,1074,1072,1085,1080,1080,32,1090,1077,1084,1099,32,1089,32,71,105,116,72,117,98,58,32) -join "") + $_) -ForegroundColor Red
        Read-Host (([char[]](10,1053,1072,1078,1084,1080,1090,1077,32,69,110,116,101,114,32,1076,1083,1103,32,1074,1099,1093,1086,1076,1072,46,46,46) -join ""))
        Exit
    }
    ...
    Write-Host (([char[]](1059,1089,1090,1072,1085,1086,1074,1082,1072,32,1090,1077,1084,1099,32,1074,32,1087,1072,1087,1082,1091,32,1089,1082,1080,1085,1086,1074,32,83,116,101,97,109,46,46,46) -join "")) -ForegroundColor Cyan
    try {
        # The folder structure in the ZIP is: fluenty-luatools-patcher-main/fluenty
        $extracted_theme_dir = Join-Path $temp_extract "fluenty-luatools-patcher-main\fluenty"
        if (-not (Test-Path $extracted_theme_dir)) {
            ...
            if ($subdirs.Count -gt 0) {
                $extracted_theme_dir = Join-Path $subdirs[0].FullName "fluenty"
            }
        }
        ...
        Write-Host (([char[]](1058,1077,1084,1072,32,70,108,117,101,110,116,121,32,1091,1089,1087,1077,1096,1085,1086,32,1079,1072,1075,1088,1091,1078,1077,1085,1072,32,1089,32,1074,1072,1096,1077,1075,1086,32,71,105,116,72,117,98,32,1080,32,1091,1089,1090,1072,1085,1086,1074,1083,1077,1085,1072,33) -join "")) -ForegroundColor Green
    } catch {
...
try {
    ...
    try {
        ...
        Write-Host (([char[]](32,32,91,45,93,32,1053,1077,32,1091,1076,1072,1083,1086,1089,1100,32,1089,1082,1086,1087,1080,1088,1086,1074,1072,1090,1100,32,1082,1086,1076,32,1074,32,1073,1091,1092,1077,1088,32,1086,1073,1084,1077,1085,1072,46,32,1042,1099,32,1084,1086,1078,1077,1090,1077,32,1089,1082,1086,1087,1080,1088,1086,1074,1072,1090,1100,32,1077,1075,1086,32,1074,1088,1091,1095,1085,1091,1102,32,1080,1079,32,1082,1086,1085,1089,1086,1083,1080,32,1085,1080,1078,1077,58) -join "")) -ForegroundColor Yellow
        Write-Host "----------------------------------------------------------"
        Write-Host $css_content
        Write-Host "----------------------------------------------------------"
    }
}
